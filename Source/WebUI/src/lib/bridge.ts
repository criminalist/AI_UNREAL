...
export async function getContinuationSummarySettings(): Promise<ContinuationSummarySettings> {
	const bridge = getBridge();
	if (bridge && typeof bridge.getcontinuationsummarysettings === 'function') {
		let timer: ReturnType<typeof setTimeout> | null = null;
		const timeoutPromise = new Promise<never>((_, reject) => {
			timer = setTimeout(() => reject(new Error('getContinuationSummarySettings timed out')), 8000);
		});
		try {
...
export async function setContinuationSummaryProvider(provider: 'openrouter' | 'local'): Promise<void> {
	const bridge = getBridge();
	if (bridge && typeof bridge.setcontinuationsummaryprovider === 'function') {
...
export async function setContinuationSummaryModel(modelId: string): Promise<void> {
	const bridge = getBridge();
	if (bridge && typeof bridge.setcontinuationsummarymodel === 'function') {
...
export async function setContinuationSummaryDefaultDetail(detail: 'compact' | 'detailed'): Promise<void> {
	const bridge = getBridge();
	if (bridge && typeof bridge.setcontinuationsummarydefaultdetail === 'function') {
...
export type AgentLogoutSupport = {
	supported: boolean;
	commandPreview?: string;
	reason?: string;
};

export type AgentLogoutResult = {
	success: boolean;
	error?: string;
};
...
/** Check whether logout is supported for this agent */
export async function getAgentLogoutSupport(agentName: string): Promise<AgentLogoutSupport> {
	const bridge = getBridge();
	if (bridge) {
		const result = await bridge.getagentlogoutsupport(agentName);
		return parseResult(result);
	}
	return { supported: false, reason: 'Bridge unavailable' };
}

/** Start agent logout flow in external terminal */
export async function startAgentLogout(agentName: string): Promise<AgentLogoutResult> {
	const bridge = getBridge();
	if (bridge) {
		const result = await bridge.startagentlogout(agentName);
		return parseResult(result);
	}
...
export async function getIndexingSettings(): Promise<IndexingSettings> {
	const bridge = getBridge();
	if (bridge && typeof bridge.getindexingsettings === 'function') {
...
export async function getIndexingStatus(): Promise<IndexingStatus> {
	const bridge = getBridge();
	if (bridge && typeof bridge.getindexingstatus === 'function') {
...
export async function getSourceControlStatus(): Promise<SourceControlStatus> {
	const bridge = getBridge();
	if (bridge && typeof bridge.getsourcecontrolstatus === 'function') {
...
export async function openSourceControlChangelist(): Promise<void> {
	const bridge = getBridge();
	if (bridge && typeof bridge.opensourcecontrolchangelist === 'function') {
...
export async function openSourceControlSubmit(): Promise<void> {
	const bridge = getBridge();
	if (bridge && typeof bridge.opensourcecontrolsubmit === 'function') {
