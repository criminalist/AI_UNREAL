...
public:
	...
	/** Returns JSON: {supported, commandPreview?, reason?} */
	UFUNCTION()
	FString GetAgentLogoutSupport(const FString& AgentName);

	/** Start agent logout in an external terminal. Returns JSON: {success, error?}. */
	UFUNCTION()
	FString StartAgentLogout(const FString& AgentName);
