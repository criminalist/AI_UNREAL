...
static FString MakeSafeFileStem_Client(const FString& Value)
{
	FString Result;
...
static bool TryGetJsonRpcMessageId(const TSharedPtr<FJsonObject>& Object, int32& OutId)
{
	if (!Object.IsValid())
	{
		return false;
	...
	{
		const FString IdString = IdValue->AsString().TrimStartAndEnd();
		if (IdString.IsNumeric())
		{
			OutId = FCString::Atoi(*IdString);
			return true;
		}
	}
...
#if PLATFORM_WINDOWS
		{
			// npx/npm bootstrap can corrupt shared cache when many adapters launch in parallel.
			// Isolate cache per agent process to avoid lock contention.
			const FString ExeName = FPaths::GetCleanFilename(Config.ExecutablePath).ToLower();
			const bool bUsesNpmRuntime = ExeName == TEXT("npx") || ExeName == TEXT("npx.cmd")
				|| ExeName == TEXT("npm") || ExeName == TEXT("npm.cmd")
				|| ExeName == TEXT("npm.exe") || ExeName == TEXT("npx.exe");
			if (bUsesNpmRuntime)
			{
				const FString UserProfile = FPlatformMisc::GetEnvironmentVariable(TEXT("USERPROFILE"));
				if (!UserProfile.IsEmpty())
				{
					const FString AgentCacheDir = FPaths::Combine(
						UserProfile,
						TEXT(".agentintegrationkit"),
						TEXT("npm-cache"),
						MakeSafeFileStem_Client(Config.AgentName));
					IFileManager::Get().MakeDirectory(*AgentCacheDir, true);
					ApplyTemporaryEnvVar(TEXT("npm_config_cache"), AgentCacheDir, RestoreList);
				}
			}
		}
#endif
...
void FACPClient::Disconnect()
{
	...
	// Clear state
	{
		...
		PendingInboundRequestIds.Empty();
		NextInboundRequestId = -1;
...
void FACPClient::StderrReaderLoop()
{
	...
	while (!bStopRequested && StderrReadPipe)
	{
		...
		while (Buffer.Split(TEXT("\n"), &Left, &Right))
		{
			...
			const FString SanitizedLine = StripAnsiCodes(Left);
			UE_LOG(LogAgentIntegrationKit, Log, TEXT("ACPClient stderr [%s]: %s"), *CurrentConfig.AgentName, *SanitizedLine);

			const FString LowerErr = SanitizedLine.ToLower();
			const bool bIsAuthStderr =
				LowerErr.Contains(TEXT("403 forbidden"))
				|| LowerErr.Contains(TEXT("not authenticated"))
				|| LowerErr.Contains(TEXT("authentication required"))
				|| LowerErr.Contains(TEXT("failed to connect to websocket"));
			if (bIsAuthStderr)
			{
				TWeakPtr<FACPClient> WeakSelf = AsShared();
				FString AuthMsg = SanitizedLine;
				AsyncTask(ENamedThreads::GameThread, [WeakSelf, AuthMsg]()
				{
					if (TSharedPtr<FACPClient> Self = WeakSelf.Pin())
					{
						Self->OnError.Broadcast(-32000, AuthMsg);
					}
				});
			}
			...
			{
				TWeakPtr<FACPClient> WeakSelf = AsShared();
				FString StatusLine = SanitizedLine;
...
void FACPClient::ProcessLine(const FString& Line)
{
	...
	TSharedPtr<FJsonValue> IdValue = JsonObject->TryGetField(TEXT("id"));
	const bool bHasIdField = IdValue.IsValid();
	const bool bHasMethod = JsonObject->HasField(TEXT("method"));

	// Server request: has both id and method. Accept string IDs and preserve exact type in response.
	if (bHasMethod && bHasIdField)
	{
		int32 RequestId = 0;
		if (TryGetJsonRpcMessageId(JsonObject, RequestId))
		{
			// Preserve string-form IDs so our response mirrors the original JSON type.
			if (IdValue->Type == EJson::String)
			{
				FScopeLock Lock(&StateLock);
				PendingInboundRequestIds.Add(RequestId, IdValue);
			}
		}
		else if (IdValue->Type == EJson::String)
		{
			FScopeLock Lock(&StateLock);
			RequestId = NextInboundRequestId--;
			PendingInboundRequestIds.Add(RequestId, IdValue);
			UE_LOG(LogAgentIntegrationKit, Verbose,
				TEXT("ACPClient: Server request has non-numeric string id '%s'; using surrogate id %d"),
				*IdValue->AsString(), RequestId);
		}
		else
		{
			UE_LOG(LogAgentIntegrationKit, Verbose,
				TEXT("ACPClient: Ignoring server request with unsupported id type: %s"),
				*Line);
			return;
		}
	...
	int32 MessageId = 0;
	const bool bHasUsableId = TryGetJsonRpcMessageId(JsonObject, MessageId);
	if (bHasIdField && !bHasUsableId)
	{
		UE_LOG(LogAgentIntegrationKit, Verbose, TEXT("ACPClient: Ignoring JSON-RPC response with non-numeric/null id: %s"), *Line);
		return;
	}
...
TSharedPtr<FJsonValue> FACPClient::ResolveInboundRequestIdValue(int32 RequestId)
{
	FScopeLock Lock(&StateLock);
	if (TSharedPtr<FJsonValue>* IdValue = PendingInboundRequestIds.Find(RequestId))
	{
		TSharedPtr<FJsonValue> Result = *IdValue;
		PendingInboundRequestIds.Remove(RequestId);
		return Result;
	}
	return nullptr;
}
		...
		if (Result.IsValid() && Result->HasField(TEXT("authMethods")))
		{
			...
			if (Result->TryGetArrayField(TEXT("authMethods"), MethodsArray) && MethodsArray)
			{
				for (const auto& Val : *MethodsArray)
				{
					...
					if (AuthMethod.Id.IsEmpty())
					{
						UE_LOG(LogAgentIntegrationKit, Warning, TEXT("ACPClient: Skipping auth method with empty id for '%s'"), *CurrentConfig.AgentName);
						continue;
					}
...
void FACPClient::Authenticate(const FString& MethodId)
{
	if (MethodId.IsEmpty())
	{
		UE_LOG(LogAgentIntegrationKit, Warning, TEXT("ACPClient: Skipping authenticate call with empty methodId"));
		return;
	}
...
void FACPClient::HandleServerRequest(int32 Id, const FString& Method, TSharedPtr<FJsonObject> Params)
{
	...
	else if (Method == TEXT("session/elicitation"))
	{
		...
		if (Elicitation.Mode.Equals(TEXT("form"), ESearchCase::IgnoreCase) && Params->HasField(TEXT("requestedSchema")))
		{
		...
		else if (Elicitation.Mode.Equals(TEXT("url"), ESearchCase::IgnoreCase))
		{
			Params->TryGetStringField(TEXT("url"), Elicitation.Url);
			Params->TryGetStringField(TEXT("elicitationId"), Elicitation.ElicitationId);

			if (!Elicitation.Url.IsEmpty())
			{
				const FString UrlToOpen = Elicitation.Url;
				UE_LOG(LogAgentIntegrationKit, Log, TEXT("ACPClient: Opening auth URL in browser: %s"), *UrlToOpen);
				AsyncTask(ENamedThreads::GameThread, [UrlToOpen]()
				{
					FPlatformProcess::LaunchURL(*UrlToOpen, nullptr, nullptr);
				});

				// Acknowledge URL elicitation so the agent can continue auth flow.
				RespondToElicitation(Id, TEXT("accept"), nullptr);
				return;
			}
		}
...
void FACPClient::RespondToElicitation(int32 RequestId, const FString& Action, TSharedPtr<FJsonObject> Content)
{
	...
	if (TSharedPtr<FJsonValue> OriginalId = ResolveInboundRequestIdValue(RequestId))
	{
		Response->SetField(TEXT("id"), OriginalId);
	}
	else
	{
