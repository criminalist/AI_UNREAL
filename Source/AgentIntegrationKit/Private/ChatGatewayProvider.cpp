...
TArray<FACPModelInfo> FOmniRouteProvider::GetHardcodedModels() const
{
	TArray<FACPModelInfo> Models;

	{ FACPModelInfo M; M.ModelId = TEXT("if/kimi-k2-thinking"); M.Name = TEXT("Kimi K2 Thinking"); M.Description = TEXT("Default OmniRoute reasoning model"); Models.Add(M); }

	return Models;
}

TArray<FACPModelInfo> FOmniRouteProvider::ParseModelsResponse(const FString& ResponseBody) const
{
...
void FOmniRouteProvider::ConfigureRequestHeaders(TSharedRef<IHttpRequest, ESPMode::ThreadSafe> Request, const FString& ApiKey) const
{
	// OmniRoute may run without auth (local mode), but supports Bearer tokens when configured.
	if (!ApiKey.IsEmpty())
	{
		Request->SetHeader(TEXT("Authorization"), FString::Printf(TEXT("Bearer %s"), *ApiKey));
	}
...
bool FOmniRouteProvider::ModelSupportsReasoning(const FString& ModelId) const
{
	const FString LowerModel = ModelId.ToLower();
	return LowerModel.Contains(TEXT("thinking")) || LowerModel.Contains(TEXT("reason")) || LowerModel.Contains(TEXT("r1"));
}
...
namespace ChatGateway
{
	...
	static void EnsureInitialized()
	{
		...
		ProviderInstances.Add(MakeShared<FOmniRouteProvider>());
