...
bool FMCPServer::Start(int32 Port)
{
	...
	LegacyRouteHandleSlash = HttpRouter->BindRoute(
		FHttpPath(TEXT("/mcp/")),
		EHttpServerRequestVerbs::VERB_POST,
		FHttpRequestHandler::CreateRaw(this, &FMCPServer::HandleStreamableHTTPRequest)
	);
	...
	StreamableHTTPGetHandleSlash = HttpRouter->BindRoute(
		FHttpPath(TEXT("/mcp/")),
		EHttpServerRequestVerbs::VERB_GET,
		FHttpRequestHandler::CreateRaw(this, &FMCPServer::HandleStreamableHTTPGet)
	);
	...
	OptionsRouteHandleSlash = HttpRouter->BindRoute(
		FHttpPath(TEXT("/mcp/")),
		EHttpServerRequestVerbs::VERB_OPTIONS,
		FHttpRequestHandler::CreateRaw(this, &FMCPServer::HandleOptionsRequest)
	);
	...
	DeleteRouteHandleSlash = HttpRouter->BindRoute(
		FHttpPath(TEXT("/mcp/")),
		EHttpServerRequestVerbs::VERB_DELETE,
		FHttpRequestHandler::CreateRaw(this, &FMCPServer::HandleStreamableHTTPDelete)
	);
...
void FMCPServer::Stop()
{
	...
	if (HttpRouter.IsValid())
	{
		...
		if (LegacyRouteHandleSlash.IsValid())
		{
			HttpRouter->UnbindRoute(LegacyRouteHandleSlash);
		}
		if (StreamableHTTPGetHandle.IsValid())
		{
			HttpRouter->UnbindRoute(StreamableHTTPGetHandle);
		}
		if (StreamableHTTPGetHandleSlash.IsValid())
		{
			HttpRouter->UnbindRoute(StreamableHTTPGetHandleSlash);
		}
		if (OptionsRouteHandle.IsValid())
		{
			HttpRouter->UnbindRoute(OptionsRouteHandle);
		}
		if (OptionsRouteHandleSlash.IsValid())
		{
			HttpRouter->UnbindRoute(OptionsRouteHandleSlash);
		}
		if (DeleteRouteHandle.IsValid())
		{
			HttpRouter->UnbindRoute(DeleteRouteHandle);
		}
		if (DeleteRouteHandleSlash.IsValid())
		{
			HttpRouter->UnbindRoute(DeleteRouteHandleSlash);
		}
	}
...
{
	return Version == TEXT("2024-10-07")
...
void FMCPServer::RegisterDefaultTools()
{
	...
	for (const FString& Name : ToolNames)
	{
		...
		// Create handler that bridges to tool
...
bool FMCPServer::HandleSSERequest(const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
{
	...
	// If the legacy-prefixed endpoint (/mcp/sse) was used, return the matching
	// legacy-prefixed message URL so older clients stay on the same path family.
	bool bUseLegacyPrefixedMessageRoute = false;
	const FString RequestPath = Request.RelativePath.GetPath();
	if (RequestPath.StartsWith(TEXT("/mcp/sse"), ESearchCase::IgnoreCase))
	{
		bUseLegacyPrefixedMessageRoute = true;
	}

	const FString EndpointUrl = bUseLegacyPrefixedMessageRoute
		? FString::Printf(TEXT("/mcp/message?sessionId=%s"), *SessionId)
		: FString::Printf(TEXT("/message?sessionId=%s"), *SessionId);
...
TSharedPtr<FJsonObject> FMCPServer::ProcessJsonRpcRequest(const TSharedPtr<FJsonObject>& Request)
{
	...
	else if (Method == TEXT("resources/subscribe") || Method == TEXT("resources/unsubscribe"))
	{
		// No-op for now: we expose no resources yet, but acknowledge the request.
...
TSharedPtr<FJsonObject> FMCPServer::HandleInitialize(TSharedPtr<FJsonValue> Id, TSharedPtr<FJsonObject> Params)
{
	...
	if (Capabilities.bSupportsResources)
	{
		TSharedPtr<FJsonObject> ResourcesObj = MakeShared<FJsonObject>();
		CapabilitiesObj->SetObjectField(TEXT("resources"), ResourcesObj);
	}
...
TSharedPtr<FJsonObject> FMCPServer::HandleResourcesList(TSharedPtr<FJsonValue> Id, TSharedPtr<FJsonObject> Params)
{
	TSharedPtr<FJsonObject> Result = MakeShared<FJsonObject>();
...
TSharedPtr<FJsonObject> FMCPServer::HandleResourceTemplatesList(TSharedPtr<FJsonValue> Id, TSharedPtr<FJsonObject> Params)
{
	TSharedPtr<FJsonObject> Result = MakeShared<FJsonObject>();
