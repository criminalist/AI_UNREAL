...
{
	auto TryFindClassNoShortWarning = [](const FString& Name) -> UClass*
	{
		if (Name.IsEmpty())
		{
			return nullptr;
		...
		// Avoid TryFindTypeSlow on short names (UE emits warning callstacks for those).
		if (Name.Contains(TEXT("/")) || Name.Contains(TEXT(".")))
		{
			return UClass::TryFindTypeSlow<UClass>(Name);
		}
		return nullptr;
	};

	UClass* Cls = TryFindClassNoShortWarning(ClassName);
	if (!Cls && !ClassName.StartsWith(TEXT("U")))
		Cls = TryFindClassNoShortWarning(TEXT("U") + ClassName);
	if (!Cls && !ClassName.StartsWith(TEXT("A")))
		Cls = TryFindClassNoShortWarning(TEXT("A") + ClassName);
...
{
	auto TryFindEnumNoShortWarning = [](const FString& Name) -> UEnum*
	{
		if (Name.IsEmpty())
		{
			return nullptr;
		...
		if (Name.Contains(TEXT("/")) || Name.Contains(TEXT(".")))
		{
			return UClass::TryFindTypeSlow<UEnum>(Name);
		}
		return nullptr;
	};

	UEnum* Enum = TryFindEnumNoShortWarning(EnumName);
	if (!Enum && !EnumName.StartsWith(TEXT("E")))
		Enum = TryFindEnumNoShortWarning(TEXT("E") + EnumName);
...
{
	auto TryFindStructNoShortWarning = [](const FString& Name) -> UScriptStruct*
	{
		if (Name.IsEmpty())
		{
			return nullptr;
		...
		if (Name.Contains(TEXT("/")) || Name.Contains(TEXT(".")))
		{
			return UClass::TryFindTypeSlow<UScriptStruct>(Name);
		}
		return nullptr;
	};

	UScriptStruct* Struct = TryFindStructNoShortWarning(StructName);
	if (!Struct && !StructName.StartsWith(TEXT("F")))
		Struct = TryFindStructNoShortWarning(TEXT("F") + StructName);
...
static bool ResolveSingleType(const FString& TypeStr, FName& OutCategory, FName& OutSubCategory, TWeakObjectPtr<UObject>& OutSubCategoryObject)
{
	...
	if (LowerType == TEXT("object") || LowerType == TEXT("uobject"))
	{
		OutCategory = UEdGraphSchema_K2::PC_Object;
		OutSubCategoryObject = UObject::StaticClass();
		return true;
	}
...
USCS_Node* NeoBlueprint::AddComponent(UBlueprint* BP, const FString& ComponentName, const FString& ComponentClassName, const FString& ParentName)
{
	...
	// Resolve component class via robust resolver to avoid TryFindType warnings on short names.
	UClass* ComponentClass = FindClassRobust(ComponentClassName);
