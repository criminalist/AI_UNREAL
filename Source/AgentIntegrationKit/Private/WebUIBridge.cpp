...
FString UWebUIBridge::GetPrerequisiteStatus()
{
	...
	auto CheckTool = [&](const FString& Name, const FString& ExecutableName, const FString& VersionFlag)
	{
		...
		if (bFound)
		{
			...
			int32 ReturnCode = -1;
#if PLATFORM_WINDOWS
			const FString LowerPath = ResolvedPath.ToLower();
			const bool bNeedsCmdWrapper =
				LowerPath.EndsWith(TEXT(".cmd")) ||
				LowerPath.EndsWith(TEXT(".bat")) ||
				!LowerPath.EndsWith(TEXT(".exe"));
			if (bNeedsCmdWrapper)
			{
				const FString Inner = FString::Printf(TEXT("\"%s\" %s"), *ResolvedPath, *VersionFlag);
				const FString CmdArgs = FString::Printf(TEXT("/S /C \"%s\""), *Inner);
				FPlatformProcess::ExecProcess(TEXT("cmd.exe"), *CmdArgs, &ReturnCode, &StdOut, &StdErr);
			}
			else
			{
...
FString UWebUIBridge::GetAuthMethods(const FString& AgentName)
{
	...
	for (const FACPAuthMethod& M : Methods)
	{
		...
		Obj->SetStringField(TEXT("type"), M.Type);
...
FString UWebUIBridge::GetAgentLogoutSupport(const FString& AgentName)
{
	TSharedRef<FJsonObject> Result = MakeShared<FJsonObject>();

	FString CommandPreview;
	FString Reason;
	const bool bSupported = FACPAgentManager::Get().CanLogoutAgent(AgentName, CommandPreview, Reason);

	Result->SetBoolField(TEXT("supported"), bSupported);
	if (!CommandPreview.IsEmpty())
	{
		Result->SetStringField(TEXT("commandPreview"), CommandPreview);
	}
	if (!Reason.IsEmpty())
	{
...
FString UWebUIBridge::StartAgentLogout(const FString& AgentName)
{
	TSharedRef<FJsonObject> Result = MakeShared<FJsonObject>();

	FString CommandPreview;
	FString Reason;
	if (!FACPAgentManager::Get().CanLogoutAgent(AgentName, CommandPreview, Reason))
	{
		Result->SetBoolField(TEXT("success"), false);
		Result->SetStringField(TEXT("error"), Reason.IsEmpty() ? TEXT("Logout is not supported for this agent.") : Reason);
		return JsonToString(Result);
	}

	FACPAgentManager::Get().LogoutAgent(AgentName);
	Result->SetBoolField(TEXT("success"), true);
	return JsonToString(Result);
}
...
void UWebUIBridge::BindOnSessionListUpdated(FWebJSFunction Callback)
{
	...
	// Refresh only already-connected agents here.
	// Avoid auto-connecting all installed npx/uvx agents at once: that triggers
	// parallel package bootstrap and npm cache lock/corruption errors.
	int32 RefreshedConnectedCount = 0;
	for (const FString& AgentName : AllAgents)
	{
		...
		{
			AgentMgr.RequestSessionList(AgentName);
			RefreshedConnectedCount++;
			continue;
		}
	}
	UE_LOG(LogAgentIntegrationKit, Log, TEXT("WebUIBridge: Session list refresh requested for %d connected agents"), RefreshedConnectedCount);
}
...
FString UWebUIBridge::RefreshSessionList()
{
	...
	Result->SetNumberField(TEXT("refreshedConnectedCount"), RefreshedConnectedCount);
	return JsonToString(Result);
}
