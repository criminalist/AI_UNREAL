...
static bool LuaObjectToPropertyString(const sol::object& ValueObj, FString& OutValue)
{
	if (ValueObj.is<std::string>())
	{
		OutValue = UTF8_TO_TCHAR(ValueObj.as<std::string>().c_str());
		return true;
	}
	if (ValueObj.is<bool>())
	{
		OutValue = ValueObj.as<bool>() ? TEXT("true") : TEXT("false");
		return true;
	}
	if (ValueObj.is<double>())
	{
		...
		{
			OutValue = FString::Printf(TEXT("%lld"), static_cast<long long>(V));
		}
		else
		{
			OutValue = FString::Printf(TEXT("%g"), V);
		}
		return true;
	}
	if (ValueObj.is<sol::table>())
	{
		...
		if (T["x"].valid() || T["X"].valid())
		{
			...
			OutValue = FString::Printf(TEXT("(X=%g,Y=%g,Z=%g)"), X, Y, Z);
			return true;
		}
		if (T["pitch"].valid() || T["Pitch"].valid())
		{
			double P = T.get_or("pitch", T.get_or("Pitch", 0.0));
			double Y = T.get_or("yaw", T.get_or("Yaw", 0.0));
			double R = T.get_or("roll", T.get_or("Roll", 0.0));
			OutValue = FString::Printf(TEXT("(Pitch=%g,Yaw=%g,Roll=%g)"), P, Y, R);
			return true;
		}
		if (T["r"].valid() || T["R"].valid())
		{
			...
			OutValue = FString::Printf(TEXT("(R=%g,G=%g,B=%g,A=%g)"), R, G, B, A);
			return true;
		}
	}
	return false;
...
// Arbitrary metadata (set or remove)
			...
			sol::this_state S) -> sol::object
		{
			...
			// Convert value to string (supports string/number/bool and common struct tables).
			FString FValue;
			if (!LuaObjectToPropertyString(ValueObj, FValue))
			{
		...
		// bp:configure_component(name, params) — set multiple component template properties
		// Supports both direct keys {FieldOfView=90} and nested bag {property={FieldOfView=90}}
		BP.set_function("configure_component", [&Session, FPath](sol::table /*self*/,
			const std::string& CompName, sol::table Params, sol::this_state S) -> sol::object
		{
			FString FComp = UTF8_TO_TCHAR(CompName.c_str());
			FBlueprintInfo Info = NeoBlueprint::LoadBlueprint(FPath);
			if (!Info.Blueprint)
			{
				Session.Log(TEXT("[FAIL] configure_component -> blueprint not found"));
				return sol::lua_nil;
			}

			UObject* TargetObj = NeoBlueprint::GetComponentTemplate(Info.Blueprint, FComp);
			if (!TargetObj)
			{
				Session.Log(FString::Printf(TEXT("[FAIL] configure_component(\"%s\") -> component not found"), *FComp));
				return sol::lua_nil;
			}

			int32 ChangedCount = 0;
			auto ApplyPropertyBag = [&](sol::table PropertyBag)
			{
				for (auto& Pair : PropertyBag)
				{
					...
					FString PropValue;
					if (!LuaObjectToPropertyString(Pair.second, PropValue))
					{
						continue;
					}

					FString Error;
					if (NeoBlueprint::SetObjectProperty(TargetObj, PropName, PropValue, Error))
					{
						ChangedCount++;
					}
					else
					{
						Session.Log(FString::Printf(TEXT("[WARN] configure_component(\"%s\") -> %s"), *FComp, *Error));
					}
				}
			};

			sol::object NestedPropertyObj = Params["property"];
			if (NestedPropertyObj.valid() && NestedPropertyObj.get_type() == sol::type::table)
			{
				ApplyPropertyBag(NestedPropertyObj.as<sol::table>());
			}
			else
			{
				ApplyPropertyBag(Params);
			}

			if (ChangedCount == 0)
			{
				Session.Log(FString::Printf(TEXT("[FAIL] configure_component(\"%s\") -> no properties changed"), *FComp));
				return sol::lua_nil;
			}

			FBlueprintEditorUtils::MarkBlueprintAsStructurallyModified(Info.Blueprint);
			Session.Log(FString::Printf(TEXT("[OK] configure_component(\"%s\") -> %d properties changed"), *FComp, ChangedCount));
			return sol::make_object(S, true);
		});
		...
		BP.set_function("help", [&Session](sol::table self, sol::this_state S) -> sol::object
		{
			...
			std::string Out =
				...
				"  configure_component(name, params) — set component properties\n"
