...
TArray<FACPAgentConfig> UACPSettings::GetAgentConfigs() const
{
	...
	// OpenRouter (built-in, native C++ - no external executable required)
	{
		...
		Config.RegistryId = TEXT("openrouter");
	...
	// OmniRoute (built-in, native C++ - local OpenAI-compatible gateway)
	{
		FACPAgentConfig Config;
		Config.AgentName = TEXT("OmniRoute");
		Config.RegistryId = TEXT("omniroute");
		Config.bIsBuiltIn = true;
		Config.InstallInstructions = TEXT("Built-in agent. Start OmniRoute locally and configure provider URL/API key in Settings if needed.");
		Config.ExecutablePath = TEXT("");
		Config.ApiKey = GetProviderApiKey(TEXT("omniroute"));
		Config.ModelId = TEXT("if/kimi-k2-thinking");
		Config.WorkingDirectory = FPaths::ProjectDir();

		if (const TSharedPtr<IChatGatewayProvider> OmniProvider = ChatGateway::FindProvider(TEXT("omniroute")))
		{
			const FString DefaultModel = OmniProvider->GetDefaultModel();
...
const TArray<FString>& UACPSettings::GetProviderPriority() const
{
	...
	{
		static const TArray<FString> Default = { TEXT("openrouter"), TEXT("omniroute") };
		return Default;
	}
...
void UACPSettings::AddProviderToPriority(const FString& ProviderId)
{
	if (!ProviderPriority.Contains(ProviderId))
	{
		...
		if (ProviderPriority.Num() == 0)
		{
			...
			ProviderPriority.Add(TEXT("omniroute"));
		}
		if (!ProviderPriority.Contains(ProviderId))
		{
